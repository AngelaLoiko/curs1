...
class Photo:
    ...
    Объекты этого класса добавляются в список. поэтому класс заводим отдельный, это другая сущность - объекты, добавляемые на яндекс-диск
    ...
    В этой реализации обошлись добавлением объектов класса в список, сортировать можно стандартной функцией sorted()
    определив поле сортировки в статическом методе sort_key(s),
    где s-любое поле класса, в нашем случае размер фотографии (height * width )
    @staticmethod
    def sort_key(s):
        return s.size
    """
